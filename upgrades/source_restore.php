...
if (empty($_REQUEST['uak']) || $uak == "%RESTORE_KEY%" || $_REQUEST['uak'] != $uak) {
    die('Access denied');
}
...
<p>System will be reverted to version <b>{$restore_data['backup']['created_on_version']}</b> using backup created at <b>{{$restore_data['backup']['created_at']}}</b>.</p>
...
Registry::set('config.dir.cache_templates', $config['dir']['cache_templates']);
...
/**
 * Class RestoreDataKeeper
 * @package Tygh
 */
class RestoreDataKeeper extends DataKeeper
{
    /**
     * @inheritdoc
     */
...
$result = RestoreDataKeeper::restore($backup_filename);

if ($result === true) {
    $url = $config['http_location'] . '/' . $config['admin_index'] . '?cc=1&ctpl=1';
    ...
    if (!empty($result)) {
        fn_echo("<br><strong>{$result}</strong>");
    }
}
