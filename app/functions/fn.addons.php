...
function fn_check_addon_snapshot($addon)
{
    ...
    if ($mode == strrev('eerf') && in_array(md5($addon), $addons_snapshots)) {
        $status = false;
    } elseif ($mode == strrev('lairt') && in_array(md5($addon . ':' . $mode), $addons_snapshots)) {
        $status = false;
    }
...
function fn_get_addons($params, $items_per_page = 0, $lang_code = CART_LANGUAGE)
{
    ...
    $installed_addons = db_get_hash_array(
        'SELECT a.addon, a.status, b.name as name, b.description as description, a.separate, a.unmanaged, a.has_icon, a.install_datetime '
