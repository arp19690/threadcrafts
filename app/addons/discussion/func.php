...
function fn_get_discussions($params, $items_per_page = 0)
{
    ...
    $posts = db_get_array(
        "SELECT " . implode(',', $fields) . " FROM ?:discussion_posts $join WHERE 1 $condition $sorting $limit"
    );
...
function fn_discussion_get_object($params)
{
    ...
    return db_get_row("SELECT thread_id, object_type, object_id, type FROM ?:discussion WHERE " . implode(' AND ', $condition));
}
