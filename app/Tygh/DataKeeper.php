...
class DataKeeper
{
    ...
    public static function backup($params = array())
    {
        ...
        if (!empty($params['compress'])) {
            ...
            if ($params['compress'] == 'zip') {
                $result = self::createZipArchive($destination_path . $pack_name . $ext, $destination_path . $pack_name);
            } else {
    ...
    public static function backupFiles($params = array())
    {
        ...
        if (!empty($params['fs_compress'])) {
            ...
            if ($params['fs_compress'] == 'zip') {
                $result = self::createZipArchive(
                    fn_get_cache_path(false) . 'tmp/backup/_files/' . $pack_name . $ext,
                    fn_get_cache_path(false) . 'tmp/backup/_files/' . $pack_name
                );
            } else {
    ...
    public static function getBackupType($backup_filepath, $throw_exception = false)
    {
        ...
        if ($extension == 'sql') {
            ...
            try {
                $files_list = self::getCompressedFilesList($backup_filepath, false);
            } catch (\Exception $e) {
                if ($throw_exception) {
                    throw $e;
                }
        ...
        switch ($ext) {
            case 'zip':
                ...
                if (!$validators->isZipArchiveAvailable()) {
                    throw new \Exception(__('error_zip_php_extension_not_installed'));
                }
